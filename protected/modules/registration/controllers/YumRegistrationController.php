...
class YumRegistrationController extends YumController {
	public $defaultAction = 'registration';
        ...
        /**
         * property to hold the type of account.
         */
        private $_accountType = YumRegistration::DEFAULT_ACCOUNT_TYPE;//variable to hold the value for User Account Type (Individual, Agent, Builder). Default is Individual.
        
        public function filters()
	{
            return array(
                'accessControl', // perform access control for CRUD operations
                'chkRegInit + registration',//check regitration initials. User Account Type must be set before filling the registration form
            );
	}
	...
	public function accessRules() {
		return array(
				array('allow',
					'actions' => array('index', 'registration', 'recovery', 'activation', 'resendactivation','accounts'),
					'users' => array('*'),
					),
				array('allow',
		...
		if (isset($_POST['YumRegistrationForm'])) { 
			...
			if(!$form->hasErrors() && !$profile->hasErrors()) {
				...
				$user->register($form->username, $form->password, $profile);
                                $user->user_type = $this->_accountType;
                                
                                $mail_sent = $this->sendRegistrationEmail($user);
                                if($mail_sent){
                                    $user->status = YumUser::STATUS_VERIFICATION_PENDING;
                                    $user->save();
                                }
	...
	public function sendRegistrationEmail($user) {
		...
		$body = strtr(
			...
			//$sent = YumMailer::send($mail);
                        $sent = true; //to be removed later (DON'T FORGET TO UNCOMMENT THE COMMENT ABOVE)
	...
	public function actionActivation($email, $key) {
		...
		if($status instanceof YumUser) {
                        ...
                        if(Yum::module('registration')->enableApprovalRequired){
                            $this->redirect(array('//user/user/status', 'code'=>  $status->status));
                        }
			elseif(Yum::module('registration')->loginAfterSuccessfulActivation) {
	...
	public function actionRecovery($email = null, $key = null) {
        ...
        /**
         * display information about available user account types. 
         */
        public function actionAccounts() {
            $this->title = "Account Types";
            $accounts = YumRegistration::validAccountType();
            $reg_url = Yum::module('registration')->registrationUrl;
            $this->render('accounttype', array('accounts'=>$accounts, 'reg_url'=>$reg_url));
	}
        
        /**
         * Function checks whether a account type is selected by the user if choice is enabled.
         * It also ensures that user choose only a valid account type.  
         * 
         * @param type $filterChain
         */
        public function filterChkRegInit($filterChain){
                if(Yum::module('registration')->enableChooseAccountType){
                    $valid_type = YumRegistration::validAccountType();
                    if(isset($_GET['type']) && array_key_exists($_GET['type'], $valid_type)){
                        $this->_accountType = $_GET['type'];
                    }
                    else {
                        Yii::app()->user->setFlash('usertype', 'Please choose an account type first.' );
                        $this->redirect($this->createUrl('//registration/registration/accounts'));
                    }
                }
                $filterChain->run();
	}
