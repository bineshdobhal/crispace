...
class YumAuthController extends YumController {
	...
	public function accessRules() {
		return array(
				array('allow',
					'actions'=>array('login', 'facebook', 'captcha'),
					'users'=>array('*'),
					),
				array('allow',
	...
	public function loginByUsername() {
		...
		if($user){
                    if($user->status == YumUser::STATUS_ACTIVE)
                        return $this->authenticate($user);
                    else
                        $this->redirect(array('//user/user/status', 'code'=>  $user->status));
                }
		else {
			Yum::log( Yum::t(
	...
	public function loginByEmail() {
		if(Yum::hasModule('profile')) {
			...
			if($profile && $profile->user){
                            if($profile->user->status == YumUser::STATUS_ACTIVE)
	...
	public function actionLogin() {
		...
		if (isset($_POST['YumUserLogin'])) {
			...
			$t = Yum::module()->loginType;
                        ...
                        $login_attempts = Yii::app()->user->getState('login_attempts', 0) + 1;
                        Yii::app()->user->setState('login_attempts', $login_attempts);
